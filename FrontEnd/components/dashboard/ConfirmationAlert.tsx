...
const DeleteConfirmation: React.FC<DeleteConfirmationProps> = ({ children, text, buttonColor, handleAction, itemId }) => {
  const submit = () => {
    confirmAlert({
        customUI: ({ onClose }) => {
          return (
            ...
            <div className="relative p-4 w-full max-w-md max-h-full">
              <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div className="p-4 md:p-5 text-center">
                  ...
                  <button
                    onClick={() => {
                    handleAction(itemId.toString());
                    onClose();
                    }}
                    type="button" className={`text-white bg-${buttonColor}-600 hover:bg-${buttonColor}-800 focus:ring-4 focus:outline-none font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2`}>
                    نعم, أنا متأكد
                  </button>
