...
export default function () {
    ...
    },
    {
      ...
      cell: (row: { id: number; }) => (
        <div className="flex flex-row gap-2">
          {enrollEnable==true && <DeleteConfirmation handleDelete={handleDelete} itemId={row.id} />}
        </div>
      ),
      button: true,
    },
  ...
  const [enrollEnable, setEnrollEnable] = useState(false);
  ...
  const fetchSettings = async () => {
    ...
    setEnrollEnable(response.data.data.enroll_subjects);
  };
    ...
    <section className="min-h-screen mt-4">
      ...
      <section className="bg-gradient-to-r from-slate-50 to-slate-100 mb-4 rounded border border-gray-200 hover:shadow p-4">
        <div className="flex justify-between items-center">
          ...
          <div className="text-left">
            {
              enrollEnable==true &&
              <Link href="/dashboard/student/term/create">
      ...
      <section className="block p-2 bg-white border border-gray-200 rounded bg-gradient-to-r from-slate-50 to-slate-100">
        ...
        <div className="bg-white mb-4 rounded border-gray-200 border p-2">
          <p className="text-md font-normal text-gray-500">السجل الآتي يعرض تسجيل المقررات : </p>
        </div>
