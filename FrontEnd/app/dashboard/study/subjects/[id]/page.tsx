...
export default function ({ params }) {
  ...
  async function handleSubmit(formData: FormData) {
    const response = await api.patch(`subjects/${params.id}`, {
          ...
          <div className="bg-gradient-to-r from-slate-100 to-slate-50 shadow border border-gray-200 rounded-lg mt-4 mb-4">
            <div className="grid gap-6 mb-6 md:grid-cols-5 p-4">
              <div>
                ...
                <input minLength={2} type="text" defaultValue={
                  // @ts-ignore
                  data?.name
                } name="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5-gray-600 p-2" required />
              </div>
              ...
              <div>
                ...
                <input minLength={2} type="text" defaultValue={
                  // @ts-ignore
                  data?.code
                } name="code" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5-gray-600 p-2" required />
              </div>

              <div>
                <label className="block mb-2 text-sm font-medium text-gray-900">عدد الساعات</label>
                <input name="score" required type="number" 
                defaultValue={
                  // @ts-ignore
                  data?.score
                }
                min={1} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5-gray-600 p-2" placeholder="" />
              </div>
              ...
              <div>
                ...
                <select onChange={(e) => changeSpecialty(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5-gray-600 p-2">
                  {specialties && specialties.map((item: any) => (
                    // @ts-ignore
                    <option key={item.id} selected={item.id==data?.specialty?.id} value={item.id}>{item.name}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <select required name="user_id" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5-gray-600 p-2">
                  {doctors.map((doctor: any) => (
                    // @ts-ignore
                    <option selected={doctor.id==data?.doctor.id} key={doctor.id} value={doctor.id}>{doctor.name}</option>
                  ))}
                </select>
              </div>
            </div>
