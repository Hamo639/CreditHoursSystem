...
export default function () {
    ...
    },
    {
      ...
      cell: (row: { id: number; }) => (
        <div className="flex flex-row gap-2">
          <Link href={`/dashboard/users/supervisors/${row.id}`}>
  ...
  const fetchData = async () => {
    const response = await api.get('users/groups/supervisor');  
    ...
    <section className="min-h-screen mt-4">
      ...
      <section className="bg-gradient-to-r from-slate-50 to-slate-100 mb-4 rounded border border-gray-200 hover:shadow p-4">
        <div className="flex justify-between items-center">
          <div className="text-right">
            <h1 className="mb-4 text-2xl font-extrabold tracking-tight leading-none text-gray-900">المراقبين</h1>
            <p className="text-md mt-2 font-normal text-gray-500 dark:text-gray-400">سجل بجميع المراقبين.</p>
          </div>

          <div className="text-left">
            <Link href="/dashboard/users/supervisors/create">
      ...
      <section className="block p-2 bg-white border border-gray-200 rounded bg-gradient-to-r from-slate-50 to-slate-100">
        ...
        <div className="bg-white mb-4 rounded border-gray-200 border p-2">
          <p className="text-md font-normal text-gray-500">السجل الآتي يعرض المراقبين : </p>
        </div>
